...
starting_times <- starting_times %>%
  mutate(Extracted = basename(file_path_sans_ext(PatientID_Time)))

starting_times <- separate(starting_times, Extracted, into = c("ID", "Date"), sep = " ")

starting_times$Date[164] = as.character("(2017-11-14)60sec")

starting_times$Date <- str_extract(starting_times$Date, "\\d{4}-\\d{2}-\\d{2}")

starting_times$Unique_ID <- paste(starting_times$ID, starting_times$Date, sep = "_")
...
```{r}
starting_times <- starting_times %>%
  unite(Start, `Start Time 01:00:00`:`Start Time 15:57:00`, sep = " ")

starting_times$Start <- str_extract(starting_times$Start, "\\d{2}:\\d{2}:\\d{2}")
```
...
```{r}
starting_times$Date_Time <- paste(starting_times$Unique_ID, starting_times$Start, sep = " ")

starting_times$Date_Time <- str_extract(starting_times$Date_Time, "\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2}")

starting_times$Date_Time <- trimws(starting_times$Date_Time)
```
...
```{r}
raw_data <- raw_data %>%
  mutate(Extracted = file_path_sans_ext(filepath))

raw_data <- separate(raw_data, Extracted, into = c("ID", "Date"), sep = " ")

raw_data$ID <- str_sub(raw_data$ID, -3, -1)

raw_data$Date <- str_extract(raw_data$Date, "\\d{4}-\\d{2}-\\d{2}")
...
```{r}
raw_data$Date_Time <- paste(raw_data$Unique_ID, raw_data$Start, sep = " ")

raw_data <- raw_data %>%
  relocate(Date_Time, .before = Epoch_Number)

raw_data$Date_Time <- str_extract(raw_data$Date_Time, "\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2}")

raw_data <- raw_data %>%
  mutate(Date_Time_Formatted = ymd_hms(Date_Time))

raw_data <- raw_data %>%
  group_by(Unique_ID) %>%
  mutate(Time_Stamp = Date_Time_Formatted + seconds((Epoch_Number - 1) * 60))

raw_data$Hour <- format(raw_data$Time_Stamp , format = "%H")

raw_data <- raw_data %>%
  mutate(Days = weekdays(Time_Stamp)) %>%
...
```{r}
# # add predicted values to dataframe to visually inspect change
# # quality of life
# long_data$row_number <- 1:nrow((long_data))
# 
# model_data_QLQ_int <- model.frame(model_QLQ)
# model_data_QLQ_int$row_number <- as.integer(row.names(model_data_QLQ_int))
# model_data_QLQ_int$predicted_QLQ <- predict(model_QLQ, re.form = ~ 0)
# 
# long_data <- left_join(long_data, model_data_QLQ_int[c("row_number", "predicted_QLQ")], by = "row_number")
# 
# # fatigue
# model_data_FA_int <- model.frame(model_FA)
# model_data_FA_int$row_number <- as.integer(row.names(model_data_FA_int))
# model_data_FA_int$predicted_FA <- predict(model_FA, re.form = ~ 0)
# 
# long_data <- left_join(long_data, model_data_FA_int[c("row_number", "predicted_FA")], by = "row_number")
# 
# # VO2
# model_data_VO2_int$ID <- 101:(nrow(model_data_VO2_int) + 100)
# model_data_VO2_int$ID <- as.factor(model_data_VO2_int$ID)
# model_data_VO2_int$predicted_VO2 <- predict(model_VO2)
# 
# long_data <- left_join(long_data, model_data_VO2_int[c("ID", "predicted_VO2")], by = "ID")
```


Visualize the mean response among morning and afternoon patients in the control and intervention groups over time.


```{r}
# # quality of life
# ggplot(long_data, aes(x = timepoint, y = QLQ_summary_score)) +
#   geom_line(aes(group = ID), color = "#7F8487", size = 0.6, alpha = 0.6) +
#   geom_point(aes(y = predicted_QLQ, col = Training_Group.y)) +
#   geom_smooth(method = "loess", aes(y = predicted_QLQ, group = Training_Group.y), se = FALSE) +
#   scale_x_discrete(breaks = c("12 Weeks", "24 Weeks")) +
...
```{r, echo=TRUE, include=TRUE}
set.seed(123)
Scaled_Relative_Acceleration <- Relative_Acceleration %>% 
  select(ID, Hour, VM3.x, RA) %>% 
  mutate(Hour = as.numeric(Hour))

# Aggregate only VM3.x by ID and Hour
Scaled_Relative_Acceleration <- aggregate(cbind(VM3.x, RA) ~ ID + Hour, data = Scaled_Relative_Acceleration, FUN = mean)

# Scale only VM3.x (not Hour)
Scaled_Relative_Acceleration <- scale(Scaled_Relative_Acceleration[c("VM3.x", "RA")])

# Perform hierarchical k-means clustering on the scaled VM3.x along with Hour
hkmeans_results <- hkmeans(Scaled_Relative_Acceleration, k = 2)

# Use fviz_cluster to visualize the clusters
fviz_cluster(
  hkmeans_results, 
  data = Scaled_Relative_Acceleration,
...
Scaled_Relative_Acceleration <- as.data.frame(Scaled_Relative_Acceleration)
Scaled_Relative_Acceleration$Clusters <- hkmeans_results$cluster
```